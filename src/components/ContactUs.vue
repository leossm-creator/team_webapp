<template>
  <div class="page_container" id="contact_us">
    ...
    <!-- <v-container class="content_container">
      <h3>asdfoadsof fdioau diosf auofliudfoia fudofid</h3>
    </v-container> -->
        ...
        <v-card flat>
          <v-container class="content_container" id="cu_email">
            ...
            >
              ...
              <v-row>
                <v-col cols="2">
                  <v-select :items="fields" item-text="text" item-value="val" label="Subject" v-model="mailData.subject" :rules="[v => !!v || 'Please select the subject']" required></v-select>
                </v-col>
                <v-col cols="10">
                  <v-text-field label="Title" v-model="mailData.title" :rules="[v => !!v || 'Please fill in the title']" required></v-text-field>
                </v-col>
              </v-row>
              <v-row>
                <v-col>
                  <v-textarea label="Your Message" v-model="mailData.body" :rules="[v => !!v || 'Please fill in your message']" required rows="12"></v-textarea>
                </v-col>
        ...
        <v-card flat>
          <v-container class="content_container" id="cu_info">
            <v-row>
              ...
              <v-col cols="6">
                <div>
                  <!-- <v-img src="/img/kakao_map.png"></v-img> -->
                  <iframe
  ...
  methods: {
    mailTo() {
      if (this.$refs.mailTo.validate()) {
        if (this.mailData.title === "연승주" && this.mailData.body === "선생님" && this.mailData.subject === "etc") {
          alert("Easter Egg");
        } else {
          let address;
          let cc;
